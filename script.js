...
//Ye graph ke options h ki graph ke edges and nodes kaise honge
const options = {
    edges: {
...
//initializing the graph network for question
...
//creating data for the graph to inserted in the container
function createData() {
    ...
    //vis.js k nodes ka data dalne k liye
    nodes = new vis.DataSet(nodes)

    //Creating tree like graph structure
    let edges = []
    for (let i = 2; i <= V; i++) {
    ...
    //Randomly new edges add krte hai
    //Bus=0 and plane=1
    ...
    for (let i = 1; i <= V / 2;) {
        ...
        if (n1 !== n2) {
            ...
            //checking if these random edges already exists if yes then which kind (bus or plane)
            ...
            //works=0,can add both bus as well as plane
            //works=1,can add only plane
...
genNew.onclick = () => {
    ...
    temptext2.innerText = `Find least path from ${cities[src - 1]} to ${cities[dst - 1]}`
...
function shouldTakePlane(edges, dist1, dist2, mn_dist) {
    let plane = 0 //plane liya na nhi liya
    let p1 = -1, p2 = -1 //p1 aur p2 stations k name for plane
    for (let pos = 0; pos < edges.length; pos++) {
        let edge = edges[pos]
        if (edge['type'] === 1) {
            let to = edge['to'] - 1
            ...
            if (dist2[to][0] + wght + dist2[from][0] < mn_dist) {
                plane = wght
                p2 = to
                p1 = from
...
function solveData() {
    ...
    //question data se adj mat bnao
    const graph = createGraph(data)

    //djikstra apply kro src and dst se
    let dist1 = djikstra(graph, V, src - 1)
    let dist2 = djikstra(graph, V, dst - 1)

    //initialise min_dist to min dis via bus from src to dst
